...
*******************************************************************************
	...
	integer i,j,k,l,m,n,Nx,My,Ot,KPsi,index,kerr, kosc, report, 
     c	LL, length, TT, yy, psave,tic, ct, rcc, flag, IBL
	double precision A, Xmin, Xmax, Ymin, Ymax, xi, Umax, Re, C,
     c	dx,dx2,dxx,dy,dy2,dyy,dt,Kappa2,KappaA,Rc,Cx,Cx2,Cy,Cy2,
     c	alphaX,alphaY,alpha,Tol,OmTol,PsiTol,
     c	Amp,pi,Lambda,freq,yhalf,t

*	Parameters to be adjusted
*******************************************************************************
	parameter (Xmin=-20, Xmax=20, Ymin=1, Ymax=11)
	parameter (Nx=99, My=199, pi = 3.14159265358979, Umax = 1)
...
1	format(1X,(101D30.14))
2	format(1X,(101I30.1))
3	format(1X,(1D30.14))
4	format(1X,(1I30.1,2D30.14))
******************************************************************************* 
...
*			IBL=(My+1)/20
			...
			if (index2.eq.'y') then
				print *,'Do you want to see screen output of the,
     c				flow-field initialization? (y/n)'
				read *,idx
				goto 2000
			else
				goto 5000
...
*************************************************************************
	...
	double precision Omega(Nx+2,My+2),Psi(Nx+2,My+2),Psi0(Nx+2,My+2),
     c	Kappa2,KappaA,dxx,Tol,PsiTol,Psi1m,Psi0m,x(Nx+2),y(My+2),d2,d12,d20
...
40	enddo
	...
	do 60 j = My-1,2,-1
	d12=x(i)**2+y(j)**2
	Psi(i,j) = 1/(2*(1.0+Kappa2))*(-dxx*Omega(i,j)*d12 + 
     c	5.0*Psi0(i+1,j) - 4.0*Psi0(i+2,j) + Psi0(i+3,j) + 
     c	Kappa2*(5.0*Psi0(i,j+1) -4.0*Psi0(i,j+2)) + Psi0(i,j+3))
...
60	enddo
	...
	do 70 j = My-1,2,-1
	d20=x(i)**2+y(j)**2
	Psi(i,j) = 1/(2*(1.0+Kappa2))*(-dxx*Omega(i,j)*d20 + 
     c	5.0*Psi0(i-1,j) - 4.0*Psi0(i-2,j) + Psi0(i-3,j) +
     c	Kappa2*(5.0*Psi0(i,j+1) - 4.0*Psi0(i,j+2)) + Psi0(i,j+3))
